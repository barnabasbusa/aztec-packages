...
keystore_dir="/shared/config/keystore/"
...
if [ "$KEYSTORE_ENABLED" == "true" ]; then
  ...
  publisher_arg=""
  if [ "$WEB3_SIGNER_URL" != "" ]; then
    attester_arg=(${addrresses[*]})
    prover_publisher_arg=$address
  else 
    attester_arg=(${private_keys[*]})
    publisher_arg=$private_key
  fi

  jq -n '.schemaVersion=1 | .remoteSigner=$WEB3_SIGNER_URL | .validators.attester=$ARGS.positional' --arg WEB3_SIGNER_URL "$WEB3_SIGNER_URL" --args $attester_arg > /shared/config/keystore/validator.json
  ...
  jq -n '.schemaVersion=1 | .remoteSigner=$WEB3_SIGNER_URL | .prover.id=$PROVER_ID | .prover.publisher=$ARGS.positional' \
    ...
    --args $publisher_arg \
    > /shared/config/keystore/prover.json
else
